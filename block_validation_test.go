...
const (
	CLIENT_RESPONSE_PREVIOUS_EPOCH = `
{
            "jsonrpc": "2.0",
            "result": {
...
`
	CLIENT_BLOCK_RESPONSE_NEXT_BLOCK = `
{
        "jsonrpc": "2.0",
        "result": {
            "approvals_after_next": [
...
type DummyLiteClient struct {
	Head                  nearprimitive.LightClientBlockView
	BlockProducerPerEpoch map[nearprimitive.CryptoHash][]nearprimitive.ValidatorStakeView
}

func (d *DummyLiteClient) new_from_checkpoint(checkpoint_head nearprimitive.LightClientBlockView) {
	d.Head = checkpoint_head
	d.BlockProducerPerEpoch = map[nearprimitive.CryptoHash][]nearprimitive.ValidatorStakeView{
		checkpoint_head.InnerLite.NextEpochId: checkpoint_head.NextBps,
	}
}
...
func TestValidateLightBlock(t *testing.T) {
	...
	lc := &DummyLiteClient{}
	lc.new_from_checkpoint(pre_epoch)

	err = ValidateLightBlock(mock.MockHostFunction{}, &lc.Head, &curr_epoch, lc.BlockProducerPerEpoch)
	if err != nil {
		t.Errorf("Failed to validate block: %s", err)
	}

	err = ValidateLightBlock(mock.MockHostFunction{}, &lc.Head, &next_epoch, lc.BlockProducerPerEpoch)
	if err != nil {
		t.Errorf("Failed to validate block: %s", err)
	}
	...
	err = ValidateLightBlock(mock.MockHostFunction{}, &lc.Head, &curr_epoch, lc.BlockProducerPerEpoch)
	if err == nil {
		t.Errorf("Block verification succeded but it should not!!!")
	}
}
